# ========================================
# Totem Rituals - Help Command
# ========================================
# Shows all available commands

tellraw @s [{"text":"\n=== Totem Rituals Commands ===","color":"gold","bold":true}]
tellraw @s [{"text":"","color":"yellow"}]
tellraw @s [{"text":"  /function rituals:help","color":"aqua"},{"text":" - Show this help menu","color":"white"}]
tellraw @s [{"text":"  /function rituals:guidebook","color":"aqua"},{"text":" - Get the guidebook","color":"white"}]
tellraw @s [{"text":"  /function rituals:get","color":"aqua"},{"text":" - Interactive item menu","color":"white"}]
tellraw @s [{"text":"","color":"yellow"}]
tellraw @s [{"text":"Admin Commands:","color":"red","bold":true}]
tellraw @s [{"text":"  /function rituals:admin/unlock_all_recipes","color":"dark_red"},{"text":" - Unlock all recipes","color":"gray"}]
tellraw @s [{"text":"  /function rituals:admin/list_totems","color":"dark_red"},{"text":" - List nearby totems","color":"gray"}]
tellraw @s [{"text":"  /function rituals:admin/reset_all","color":"dark_red"},{"text":" - Reset all rituals","color":"gray"}]
tellraw @s [{"text":"  /function rituals:admin/uninstall","color":"dark_red"},{"text":" - Uninstall datapack","color":"gray"}]
tellraw @s [{"text":"================================","color":"gold","bold":true}]
