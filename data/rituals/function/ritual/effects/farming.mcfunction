...
execute if entity @s[tag=rituals.debug] run tellraw @a[distance=..10] [{"text":"[DEBUG FARM] Farming attempt now!","color":"green","bold":true}]
...
execute if entity @s[tag=rituals.debug] run tellraw @a[distance=..10] [{"text":"[DEBUG FARM] Calling tier-specific farming","color":"red","bold":true}]
