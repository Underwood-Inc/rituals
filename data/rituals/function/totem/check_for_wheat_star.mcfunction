...
execute as @e[type=interaction,tag=rituals.totem,tag=rituals.has_item,distance=..1,limit=1] at @s if entity @e[type=item_display,tag=rituals.totem_display,distance=..3,nbt={item:{id:"minecraft:wheat"}}] run function rituals:totem/tag_if_valid_breeding_supplementary
